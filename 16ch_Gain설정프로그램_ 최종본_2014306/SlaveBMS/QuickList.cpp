...
BEGIN_MESSAGE_MAP(CQuickList, CListCtrl)
	...
	//ON_MESSAGE( MSG_POST_SUBCLASS_LIST, OnPostSubclassListview )
 //ON_WM_MEASUREITEM()
 //ON_WM_SIZE()
 //ON_NOTIFY(HDN_ITEMCHANGEDA, 0, &CQuickList::OnHdnItemchanged)
 //ON_NOTIFY(HDN_ITEMCHANGEDW, 0, &CQuickList::OnHdnItemchanged)
 ////ON_NOTIFY(HDN_ITEMCHANGINGA, 0, &CQuickList::OnHdnItemchanging)
 ////ON_NOTIFY(HDN_ITEMCHANGINGW, 0, &CQuickList::OnHdnItemchanging)
 //ON_NOTIFY(HDN_BEGINTRACKA, 0, &CQuickList::OnHdnBegintrack)
 //ON_NOTIFY(HDN_BEGINTRACKW, 0, &CQuickList::OnHdnBegintrack)
 //ON_WM_SIZE()
 //ON_WM_WINDOWPOSCHANGING()
 //ON_NOTIFY(HDN_ITEMCHANGINGA, 0, &CQuickList::OnHdnItemchanging)
 //ON_NOTIFY(HDN_ITEMCHANGINGW, 0, &CQuickList::OnHdnItemchanging)
 //ON_NOTIFY(HDN_ENDTRACKA, 0, &CQuickList::OnHdnEndtrack)
 //ON_NOTIFY(HDN_ENDTRACKW, 0, &CQuickList::OnHdnEndtrack)
END_MESSAGE_MAP()
...
BOOL CQuickList::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	// PreSubclassWindow()에 앞서 PreCreateWindow()가 수행되고
	// 여기서 설정한 style이 우선시 된다.
	// DrawItem(...)을 호출하려면
...
void CQuickList::PreSubclassWindow() 
{
	...
	SetWindowLongPtr(m_hWnd, GWL_STYLE, GetWindowLongPtr(m_hWnd, GWL_STYLE)|LVS_OWNERDATA);
	CListCtrl::PreSubclassWindow();

	/// 새로 넣음 (2013-04-11)

	//PostMessage( MSG_POST_SUBCLASS_LIST, 0L, 0L );
//	CListCtrl::PreSubclassWindow();
}
...
BOOL CQuickList::SetItemHeight(int nHeight)
{
	CBitmap bitmap;
	//m_Image.Create( IDB_BITMAP_ITEM, 2, 1, 0x00ffffff);
	m_ImageHeight.Detach();
	m_ImageHeight.Create(1,nHeight,ILC_COLOR16, 0x00ffffff, 1 );
	m_ImageHeight.DragShowNolock( TRUE );
	SetImageList(&m_ImageHeight, LVSIL_SMALL);
