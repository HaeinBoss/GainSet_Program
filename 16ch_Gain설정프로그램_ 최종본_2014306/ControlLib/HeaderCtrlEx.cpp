		...
		if (rectItem.right >= rectClip.left || rectItem.left <= rectClip.right)
		{
			...
			{	//통합 제목 쓰기
				...
				{	//통합 제목 출력
					...
					pDC->DrawText(sTitle, upperRect, DT_SINGLELINE|DT_VCENTER|DT_CENTER|DT_END_ELLIPSIS);		  // 문자열 길이가 초과될때 문자열 끝부분을 "..."  로 출력함.

					// Singleline 으로 설정되어서 사용자가 원하는 라인수에 맞게 출력함.
					if (m_font.GetSafeHandle())
...
void CHeaderCtrlEx::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	TCHAR seps[] = _T("\n");
	TCHAR *token = _tcstok(lpBuffer, seps);
	while (token)
	{
		arrTitle.Add(token);
	...
	{
		titleRect.top = rect.top + rect.Height()/arrTitle.GetSize() * i;
		titleRect.bottom = titleRect.top + rect.Height()/arrTitle.GetSize();
...
{	//Header의 높이 조절은 설정된 폰트에 따라 달라진다.
	...
	nHeight -= 6; //여백 제외
	CFont font;
	font.CreatePointFont((int)(nHeight/1.4)*10, "MS Serif");
